...
function getLearnerData(CourseInfo, AssignmentGroup, LearnerSubmissions) {
  ...
  /*  // Filter out assignments (assignment ID) that are not yet due
    // If the Due date is after the current date, then filter out that assignment ID
    const currentTimestamp = Date.now();
    for (const learnerId in learnersMap) {
        const learner = learnersMap[learnerId];
        for (const assignmentId in learner.assignments) {
            const assignment1 = assignment.assignment_id;
            if (assignment1.due_at.getTime() > currentTimestamp) {
                delete learner.assignments.assignment_id;
            }
        }
    }*/
